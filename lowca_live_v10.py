...
MAX_HISTORY = 30
MEGA_SALE_RATIO = Decimal("0.30")  # <=30% poprzedniej ceny = >=70% obniżki
...
def main() -> int:
        ...
        try:
            ...
            state[key] = {
                **snapshot_dict(snapshot),
                ...
                "last_alert_signature": signature if signature != old_signature else old_signature,
                "checked_at": datetime.now(timezone.utc).isoformat(),
            }
    ...
    if alerts:
        # Send only the first new alert in this run; all candidates remain in state.
